import { LuDot, LuFolder, LuFolderTree } from "react-icons/lu";
...
export function TitlebarCwdBranchLine({
	cwd,
	branch,
	cwdIsLinkedWorktree,
}: TitlebarCwdBranchLineProps) {
	const pathTitle =
		branch && cwdIsLinkedWorktree
			? `${cwd} · ${branch} (linked worktree)`
			: branch
				? `${cwd} · ${branch}`
				: cwd;
	...
	return (
		...
		>
			{cwdIsLinkedWorktree ? (
				<LuFolderTree
					className={cn(
						"size-3.5 shrink-0 text-sky-600/85 dark:text-sky-400/70",
					)}
					aria-hidden
				/>
			) : (
